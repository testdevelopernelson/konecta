...
<body>
  <div id="loading-df" class="preload-df">
    <img src="{{ asset('img/loading_df.gif') }}">
  </div>
    ...
    .done(function (data) {
      ...
      if (hasItems) {
        ...
        items.forEach(function (item, index) {
          ...
          if (item.thumb_url) {
            var image = $('<div>').css('background-image', 'url("' +base_root+ item.thumb_url + '?timestamp=' + item.time + '")');
          } else {
...
function dialog(title, value, callback) {
  ...
  <script type="text/javascript">
    window.onload = function() {
        document.getElementById("loading-df").style.display = "none";
    };
  </script>
</body>
</html>
